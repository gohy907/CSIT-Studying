...
int codeKS(Vec2 P, float minX, float minY, float maxX, float maxY) {
    int code = 0;
...
bool clip(Vec2 &A, Vec2 &B, float minX, float minY, float maxX, float maxY) {
    int codeA = codeKS(A, minX, minY, maxX, maxY); // код области точки A
    int codeB = codeKS(B, minX, minY, maxX, maxY); // код области точки B
    while (codeA | codeB) {
